...
def analyze_demand_for_pending_orders() -> str:
    ...
    try:
        ...
        return json.dumps({
            ...
            "demandTrend": "Stable" if len(pending_orders) < 30 else "High",
...
IMPORTANT: When asked about "all orders", use analyze_demand_for_pending_orders first.

When analyzing demand:
1. Use tools to access real historical data
2. Provide specific forecasts with confidence scores
3. Identify risks and opportunities from demand patterns
...
def demand_agent(request: RequestContext) -> Dict[str, Any]:
    """AgentCore entrypoint for demand agent - matches SpendOptimo exactly."""
