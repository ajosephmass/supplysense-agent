...
def _ensure_gateway(
    ...
    *,
    ...
    for it in items:
        if it.get('name') == name:
            gateway_id = it.get('gatewayId') or it.get('id') or ''
            logger.info(f'Gateway {name} already exists with ID {gateway_id}, skipping creation')
            return gateway_id
    ...
    try:
        ...
        # Handle conflict if gateway was created between list and create calls
        if e.response.get('Error', {}).get('Code') == 'ConflictException':
            logger.warning(f'Gateway {name} already exists (race condition), fetching ID')
            items = ac.list_gateways().get('items', [])
            for it in items:
                if it.get('name') == name:
...
def _ensure_gateway_target(
    ...
    *,
    ...
    for it in items:
        if it.get('name') == name:
            target_id = it.get('targetId') or it.get('id') or ''
            logger.info(f'Gateway target {name} already exists with ID {target_id}, skipping creation')
            return target_id
    ...
    try:
        ...
        # Handle conflict if target was created between list and create calls
        if e.response.get('Error', {}).get('Code') == 'ConflictException':
            logger.warning(f'Gateway target {name} already exists (race condition), fetching ID')
            items = ac.list_gateway_targets(gatewayIdentifier=gateway_id).get('items', [])
            for it in items:
                if it.get('name') == name:
...
def _ensure_runtime(
    ...
    *,
    ...
    # Get runtime endpoint (prefer 'prod', fallback to 'DEFAULT' or any endpoint)
    ...
    try:
        ...
        logger.info(f'Found {len(eps)} runtime endpoints for {runtime_id}')
        
        # Try to find 'prod' endpoint first
        prod = next((e for e in eps if e.get('name') == 'prod'), None)
        if prod:
            endpoint_arn = prod.get('agentRuntimeEndpointArn')
            logger.info(f'Using existing prod endpoint: {endpoint_arn}')
        else:
            ...
            if default:
                ...
                logger.info(f'Using existing DEFAULT endpoint: {endpoint_arn}')
            elif eps:
                # Use any available endpoint
                endpoint_arn = eps[0].get('agentRuntimeEndpointArn')
                logger.info(f'Using first available endpoint: {endpoint_arn}')
            else:
                # No endpoints exist, try to create one
                logger.info(f'No endpoints found, creating prod endpoint for {runtime_id}')
                ...
                logger.info(f'Created prod endpoint: {endpoint_arn}')
    except Exception as e:
        logger.error(f'Failed to get runtime endpoint for {runtime_id}: {e}', exc_info=True)
        # Don't silently fail - re-raise so we get proper error in CloudFormation
        raise RuntimeError(f'Failed to get runtime endpoint for {runtime_id}: {e}') from e
    
    return runtime_id, endpoint_arn, latest_version


def _ensure_agentcore_resources(props: Dict[str, Any]) -> Dict[str, str]:
    agent_name = props.get('AgentName') or props.get('agentName') or 'SupplySense'
    instruction = props.get('SystemPrompt') or 'You are a SupplySense supply chain intelligence agent.'
    ...
    # Validate that we got a runtime endpoint ARN
    if not endpoint_arn:
        raise RuntimeError(f'Failed to create or retrieve runtime endpoint for {agent_name}. Runtime ID: {runtime_id}')
...
def handler(event, _context):
    ...
    physical_id = event.get('PhysicalResourceId') or 'SupplySenseAgentCore'
